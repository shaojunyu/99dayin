        ...
        getInfo: function getInfo() {
            ...
            this.bills.each(function () {
                var name = $(this).find('.file-name').text(),
                    //文件名
                share = $(this).find('[role="share"] input[value="yes"]').is(':checked'),
                    //是否共享
                side = $(this).find('[role="page"] input[value="single"]').is(':checked') ? 'single' : 'both',
                    //返回单双面
                direction = $(this).find('[role="direction"] input[value="column"]').is(':checked') ? 'column' : 'row',
                    ppt_select = $(this).find('[role="direction"] select'),
                    //判断是否存在select
                pptNum = -1,
                    size = $(this).find('[role="size"] select').val(),
                    //获取页面的大小值
                copies = $(this).find('[role="copies"] input').val(),
                    //获取份数
                ...
                if (method == null || method == '') {
                    ...
                    if (method === "到店自取") {
                        ...
                        info = {
                            ...
                            var school_area = $('.school-area').val(),
                                //校区
