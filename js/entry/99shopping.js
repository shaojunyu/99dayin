...
require([
    ...
    'prompt',
    'encryption'
], function ($, util, scroll, prompt, Encryption) {
    var prompt = new prompt.Prompt({ prompt: $('.prompt') });
    var Pathurl = {
        checkout: Encryption.Encryption('../index.php/api/createOrder'),
        center: Encryption.Encryption('./order'),
        back: '',
    ...
    var Delete_btn = {
        ...
        deleteItem: function deleteItem($target) {
            var li = $target.parents('tr'), mark = $target.attr('data-mark'), _this = this;
            $.ajax({
                url: Pathurl.remove,
                type: 'POST',
                data: { fileMD5: mark }
    ...
    var PayBill = {
        ...
        getInfo: function getInfo() {
            ...
            this.bills.each(function () {
                var name = $(this).find('.file-name').text(), share = $(this).find('[role="share"] input[value="yes"]').is(':checked'), side = $(this).find('[role="page"] input[value="single"]').is(':checked') ? 'single' : 'double', direction = $(this).find('[role="direction"] input[value="column"]').is(':checked') ? 'column' : 'row', ppt_select = $(this).find('[role="direction"] select'), pptNum = -1, size = $(this).find('[role="size"] select').val(), copies = $(this).find('[role="copies"] input').val(), mark = $(this).find('[role="delete"] a').attr('data-mark');
                if (ppt_select.length ? true : false) {
                    pptNum = Number(ppt_select.val());
                }
                ;
                var obj = {
                    'name': name,
        ...
        init: function init() {
            ...
            this.checkout.on('click', function () {
                _this.Files = _this.getInfo();
                var method = $('input[name="method"]:checked').val(), totle = $total_price.html();
                info = {};
                if (method == null || method == '') {
                    ...
                    if (method === '到店自取') {
                        ...
                        var school_area = $('.school-area').val(), building = $('.building').val(), phone_text = $('.phone').val().trim(), address_text = $('.address').val().trim(), reg = {
