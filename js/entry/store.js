...
'use strict';
define('checkEvent', ['jquery'], function ($) {
    Check.DEFAUlT = {};
    function Check(opts) {
        this.opts = $.extend({}, Check.DEFAULT, opts);
        this.container = this.opts.container;
        this.checkAll = this.opts.checkAll;
        this.checkBtns = this.opts.checkBtns;
...
require([
    ...
    'modal',
    'utility'
], function ($, scroll, checkEvent, prompt, modal) {
    var Iscroll = bindScroll($('.files'));
    $('.basic-btn').on('click', function (e) {
        var $target = $(e.target), _id = $target.attr('id'), $ps = $('#change-ps-page'), $withdraw_page = $('#withdraw-page');
        if (_id === 'withdraw') {
    ...
    var Withdraw = {
        amount: $('.withdraw-input'),
        confirm: $('.withdraw-confir'),
        pre_WD: $('.pre-withdraw'),
        WDed: $('.withdrawed'),
        total: $('.total'),
        calMoney: function calMoney(money) {
        ...
        init: function init() {
            ...
            this.confirm.on('click', function () {
                var money = Number(_this.amount.val()), reg = [
                        /^\d+$/,
                        /^[0-9]+(.[0-9]{1})?$/
                    ];
    ...
    var Refresh = {
        order: $('.order-header'),
        delivery_btn: $('.order-header .delivery'),
        pickup_btn: $('.order-header .pickup'),
        delivery: $('#delivery-orders'),
        pickup: $('#pickup-orders'),
        delivery_ul: $('#delivery-orders ul'),
        pickup_ul: $('#pickup-orders ul'),
        log: $('#view-log'),
        withdraw_log: $('.record'),
        changeOrder: function changeOrder() {
            ...
            this.order.on('click', function (e) {
                ...
                if ($target.hasClass('delivery')) {
                    changeShow(_this.delivery, _this.pickup);
                    changeClass($target, 'active');
                    refreshScroll(Iscroll);
                } else if ($target.hasClass('pickup')) {
                    changeShow(_this.pickup, _this.delivery);
                    changeClass($target, 'active');
                    refreshScroll(Iscroll);
                }
            });
        ...
        refreshOrder: function refreshOrder() {
            ...
            $.when($.ajax({
                ...
                if (delivery.success) {
                    _this.addLi(_this.delivery_ul, delivery.data, _this.delivery_btn);
                }
                if (pickup.success) {
                    _this.addLi(_this.pickup_ul, pickup.data, _this.pickup_ul);
                }
            });
        ...
        addLi: function addLi(ul, data, mark) {
            var li = document.createElement('li'), html = '', num = Number(mark.attr('data-order'));
            mark.attr('data-order', num);
            data.forEach(function (val, index) {
                html += '<div class="order-upart">' + '<input class="order-checkbox sub-checkbox" type="checkbox" name="order" data-num=' + (index + 1) + '>' + '<div>订单编号:<span>' + val.orderNum + '</span></div>' + '<div>收货地址:' + val.address + '</div>' + '<div><span>文件信息:</span>' + '<div class="file-info">' + '<header><span>序号</span>' + '<span>文件名</span>' + '<span>单/双面</span>' + '<span>横/竖</span>' + '<span>每页PPT数量</span>' + '<span>大小</span>' + '<span>份数</span>' + '</header>' + '<div class="container files" id="warpper' + (index + 1) + '">' + '<article id="scroller">' + function (data) {
                    var html = '';
                    data.forEach(function (val, index) {
                        html += '<div class="row" data-num="' + (index + 1) + '">' + '<span>' + (index + 1) + '</span>' + '<span><i class="' + val.classname + '"></i>' + val.name + '</span>' + '<span>' + val.pages + '</span>' + '<span>' + val.directino + '</span>' + '<span>' + val.ppt ? val.ppt : 0 + '</span>' + '<span>' + val.size + '</span>' + '<span>' + val.copies + '</span>' + '</div>';
                    });
                    return html;
                }(val.orders) + '</article>' + '</div>' + '</div>' + '</div>' + '</div>' + '<div class="summary">' + '<div class="summary-info">' + '总价\uFF1A<span>' + val.money + '\uFFE5</span> 收货方式\uFF1A' + '<span>' + val.delivery + '</span> 支付状态\uFF1A' + ' <span>未支付</span>' + '<i>剩余时间:<span class="setTime">30</span>分</i>' + '</div>' + '<img src="img/divided.png">' + ' </div>';
            });
            li.innerHTML = html;
            ul.append(li);
        },
