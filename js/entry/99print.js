...
define('scroll', ['jquery'], function ($) {
    ...
    var index = 0, length = 2, loca = 0, sec_height = 0;
    ...
    _ScrollPage.prototype.init = function (loca) {
        var transform = [
                ...
                '-moz-transition',
                'transition'
            ], _opts = this.opts, container = _opts.container, new_height = _opts.section.height();
        loca = new_height / sec_height * loca;
        if (isSuportCss(transform) && isSuportCss(transition)) {
            ...
            if (!container.is(':animated')) {
                container.animate({ top: loca }, 600);
            }
        }
    };
    $.fn.extend({
...
require.config({
    ...
    paths: {
        ...
        'SMS': 'https://cdn1.lncld.net/static/js/av-mini-0.6.1.js',
...
require([
    ...
    'encryption',
    'utility'
], function ($, scroll, modal, prompt, enroll, Encryption) {
    var scroll = new scroll.ScrollPage(), prompt = new prompt.Prompt({ prompt: $('.prompt') });
    ...
    var username_reg = [
            ...
            '用户名输入长度只能在4~12位!'
        ], pwd_reg = [/^(([a-z]+[\w]*[0-9]+)|([0-9]+[\w]*[a-z]+))[a-z0-9]*$/i], pwd_msg = [
            '密码长度在6-20位\uFF0C且必须包含数字和字母~',
            '密码长度在6-20位\uFF0C且必须包含数字和字母~'
        ], phone_reg = [/^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/], phone_msg = ['请输入正确手机号!'], college_reg = [/^[\u4e00-\u9fa5]{0,}$/], college_msg = ['输入不能包含非法字符!'];
    var Enroll = {
        $username: $('.username'),
        $pwd: $('.ps'),
        $CF_pwd: $('.confir-ps'),
        $phone: $('.phone'),
        $security_code: $('.secu-code'),
        $college: $('.college'),
        $enroll_btn: $('.enroll'),
        ...
        addDetect: function addDetect() {
            ...
            this.$CF_pwd.inputFocus().on('blur', function () {
                var ps1 = _this.$pwd.val(), ps2 = $(this).val();
                if (ps1 !== ps2) {
        ...
        send: function send() {
            ...
            this.$enroll_btn.on('click', function () {
                var sign = [], username = _this.$username.val(), pwd = _this.$pwd.val(), phone = _this.$phone.val(), college = _this.$college.val(), code = _this.$security_code.val();
                if (code == '' || code === null) {
